...
SCALAR_INDICES = [i for i in range(34) if not any(i in g for g in DIRECTION_GROUPS)]
...
def test_every_flip_is_its_own_inverse():
    for name in ("flip_x", "flip_y", "flip_diag", "flip_anti"):
        twice = np.arange(34)[_INDEX_MAPS[name]][_INDEX_MAPS[name]]
...
def test_all_eight_transforms_are_distinct_and_are_valid_permutations():
    ...
    for name in SYMMETRY_NAMES:
        ...
        assert sorted(idx) == list(range(34)), f"{name} is not a valid permutation"
        seen.add(idx)
