...
from shared.opponents import nearest_opponent, opponent_features, threatening_opponent_count
...
def stage34_potential(game_state):
    """Potential for hunting while retaining navigation and bomb survival.

    The extra term rewards progress toward the nearest living opponent and
    discourages standing in the immediate blast line of armed opponents.  It
    is used only through ``gamma * Phi(s') - Phi(s)``, so reaching a target is
    dense feedback while official game events remain the actual objective.
    """
    if game_state is None:
        return 0.0
